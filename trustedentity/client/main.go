...
func readPKIProperty (verb string, path string, itemTitle string, dataItemName string, vaultPar *api.Logical) {
	var err error
	var s *api.Secret
...
func readPKIBySerialProperty (path string, itemTitle string, dataItemName string, serialNumber string, vaultPar *api.Logical) string {
	var err error
	var s *api.Secret
	var retValue string
...
func workflow1() {
	...
	//resp, err := http.Get("http://myvault.com/")
	log.Printf("Starting HTTP request to get CA cert")
...
func workflow2() {
	...
	*/
	...
	for _, p := range pkiParams {
		readPKIProperty (p.verb, p.pathParam, p.itemTitleParam, p.dataItemNameParam, vault)
	}
	...
	if err == nil {
		...
		certificate := readPKIBySerialProperty (
			"pki/cert/",
			"Certificate",
			"certificate",
			serialNumber.(string),
...
/*
func readVaultPKIProperty (path string, itemTitle string, dataItemName string, vaultPar *api.Logical) string {
